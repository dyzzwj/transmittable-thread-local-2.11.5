...
public final class TtlRunnable implements Runnable, TtlWrapper<Runnable>, TtlEnhanced, TtlAttachments {
    ...
    private TtlRunnable(@NonNull Runnable runnable, boolean releaseTtlValueReferenceAfterRun) {
        /**
         * 此时的线程是执行ExecutorTtlWrapper#execute的线程 有可能主线程，也有可能是线程池的线程
         */
        /**
         * 这里将捕获后的父线程本地变量存储在当前对象的capturedRef里
         *  capturedRef是一个共享变量 用于在父子线程间传递数据 父线程：当前线程（调用ExecutorTtlWrapper#execute()）  子线程：线程池的线程
         */
    ...
    public void run() {
        ...
        /**                                                         父线程：（调用executorService.execute()的线程，TtlRunnbale也是由该线程创建的）
         * 重点方法replay，此方法用来给当前子线程（线程池的线程）赋本地变量(就是拿着异步发生时捕获到的父线程的本地变量，赋给自己)，返回的backup是此子线程原来就有的本地变量值（原生本地变量，下面会详细讲），
    ...
    public static TtlRunnable get(@Nullable Runnable runnable, boolean releaseTtlValueReferenceAfterRun, boolean idempotent) {
        ...
        //最终初始化 创建TtlRunnable时会把父线程本地变量存储在当前对象的capturedRef属性
